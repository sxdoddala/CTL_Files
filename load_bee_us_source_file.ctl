...
--  /*	A.Hernandez		 12/18/06	Moved VALUE15 to accept date (YYYY-MON-DD)    */
--  /*						to pass to PAY_BATCH_LINES.DATE_EARNED	    */
...
(silent=(header,feedback,discards)
 errors = 100000 )
LOAD DATA
INFILE '$CUST_TOP/data/BEE_interface/BEE_US.csv'
BADFILE '$CUST_TOP/data/BEE_interface/BEE_US.bad'
DISCARDFILE '$CUST_TOP/data/BEE_interface/BEE_US.dsc'
APPEND
...
TRAILING NULLCOLS
(
BUSINESS_GROUP constant 'TeleTech Holdings - US',
BATCH_NAME "substr(:BATCH_NAME,1,30)",
EMPLOYEE_NUMBER "rtrim(ltrim(:EMPLOYEE_NUMBER))",    
VALUE15 "trunc(to_date(:VALUE15,'YYYY-MON-DD HH24:MI:SS'))",
